...
public:
    vector<vector<int>> rotateGrid(vector<vector<int>>& grid, int k) {
        ...
        for (int layer = 0; layer < nlayer; ++layer) {
            vector<int> r, c, val;  // each element's row index, column index, and value
            ...
            for (int i = 0; i < total; ++i) {
                int idx = (i + total - kk) % total;  // the index corresponding to the
                grid[r[i]][c[i]] = val[idx];  // value after rotation
            }
        }
